---
title: "Tutorial on Pivot Tables and other Excel tasks you can also do in R with Summer of Tech event data"
author: "Kim Fitter"
output: 
  html_document: 
    toc: true
...
This tutorial assuming you have already installed R and R studio and had a brief introduction to the R basics. Run each line of code yourself or Knit to HTML to walk through this tutorial.

At the end of the exercise we will have an interactive tabular view of the event venue capacity relative to the student registration and attendance numbers.
...
**Tip: ** Install the package from CRAN the first time you use the package using install.packages("packagenamehere") else you would get an Error that there is no package.
...
This function will import to a data frame, which is a 2-dimensional object where contents can be different data types, as default.
...
In order to access the variables we can use a **$**. Alternatively we can use another method that joins objects and functions using a pipe or **%>%**. This can be useful if we would like to use multiple dplyr verbs and functions on an object. 
